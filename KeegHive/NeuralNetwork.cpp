...
namespace GANnet{
	NeuralNetwork::NeuralNetwork(EmulsionFeild * eFeild, EmulsionFeild::nnDataHolder dHolder)
	{
		...
		if (netType == Genetic) {
			...
			hiddenContainer.push_back(new NodeLayer(this, NodeTypes::HIDDEN, dHolder.hNeuronTotal.at(0).at(0), dHolder.hLayerFuncTypes.at(0), ittr)); 
			ittr++;
			...
			//Figure Out how to get data to Fprop with the genetic algo
			//Fprop(learningSet.expectedInputData);
		}
		else if (netType == Backprop) {
	...
	NeuralNetwork::~NeuralNetwork()
	{
		...
		inputContainer->~NodeLayer();
		delete inputContainer;
		for (int i; i < hiddenContainer.size() - 1; i++) {
			hiddenContainer.at(i)->~NodeLayer();
			delete hiddenContainer.at(i);
		}
		outputContainer->~NodeLayer();
		delete outputContainer;
		iTOhContainer->~SynapseLayer();
		delete iTOhContainer;
		hTOoContainer->~SynapseLayer();
		delete hTOoContainer;
